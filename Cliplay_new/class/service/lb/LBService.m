...
#pragma mark - Init
+ (id)sharedManager {
	static LBService *sharedMyManager = nil;
								 ...
								 [WBHttpRequest requestForShareAStatus:textToSend
												 ...
												 withCompletionHandler:^(WBHttpRequest *httpRequest, id result, NSError *error) {
													 if(error == nil) {
														 ...
														 [_self shareFailure:ServerError];
													 }
												 }
								  ];
							 }];
...
- (void)shareCompletesWith:(id)result error:(BOOL)error{
	...
	[progressView dismiss:NO completion:^{
		if(!error){
			MRProgressOverlayView *resultView = [_self progressViewWithText:@"Succeed" withAnimation:NO andMode:MRProgressOverlayViewModeCheckmark];
